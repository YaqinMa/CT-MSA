...
#定义了带泄露的 ReLU 激活函数，斜率参数为 0.2
def leakyrelu(x):
    return nn.functional.leaky_relu_(x, 0.2)

#CustomAct 是一个自定义激活函数类，它接收 act_layer 参数，决定使用 gelu 或 leakyrelu 激活函数。
...
class Mlp(nn.Module):
    def __init__(self, in_features, hidden_features=None, out_features=None, act_layer=leakyrelu, drop=0.):
        super().__init__()
        # 1. 设置输出维度,如果未指定则与输入维度相同
        out_features = out_features or in_features
        # 2. 设置隐藏层维度,如果未指定则与输入维度相同
        hidden_features = hidden_features or in_features
        # 3. 第一个全连接层: 输入维度 -> 隐藏维度
        self.fc1 = nn.Linear(in_features, hidden_features)
        # 4. 激活函数层: 使用CustomAct包装的激活函数(gelu或leakyrelu)
    ...
    def forward(self, x):
        # 前向传播过程:
        # 1. 第一个全连接层
        x = self.fc1(x) # shape: (B, N, in_features) -> (B, N, hidden_features)

        # 2. 激活函数
        x = self.act(x)  # 非线性变换

        # 3. Dropout(第一次)
        x = self.drop(x)  # 随机丢弃一些神经元

        # 4. 第二个全连接层
        x = self.fc2(x)  # shape: (B, N, hidden_features) -> (B, N, out_features)

        # 5. Dropout(第二次)
        x = self.drop(x)  # 再次随机丢弃

        return x

#函数生成一个自注意力机制的掩码，定义了注意力机制中每个位置能够访问的范围。
#N序列长度 w窗口大小 每个位置能看到的范围
...
class Attention(nn.Module):
    def __init__(self, dim, num_heads=8, qkv_bias=False, qk_scale=None, attn_drop=0., proj_drop=0., window_size=16):
        ...
        #创建一个可学习的参数，用于存储相对位置偏置。它的形状是 (2 * window_size - 1) * (2 * window_size - 1) 行和 num_heads 列，用于存储每个头的相对位置偏置。
        # 相对位置编码
    ...
    def forward(self, x):
        #获取输入 x 的形状，其中 B 是批次大小，N 是序列长度，C 是每个输入的特征维度。
        B, N, C = x.shape
        #通过线性层计算Q、K和 V，然后调整形状，使其成为(3, B, num_heads, N, head_dim)的张量。
...
class Block(nn.Module):
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop=0., attn_drop=0.,
        ...
        self.attn = Attention(
            dim, num_heads=num_heads, qkv_bias=qkv_bias, qk_scale=qk_scale, attn_drop=attn_drop, proj_drop=drop, window_size=window_size)        # 投影dropout
        ...
        # MLP Block
        mlp_hidden_dim = int(dim * mlp_ratio)# 隐藏层维度扩展4倍
...
class StageBlock(nn.Module):
    def __init__(self, depth, dim, num_heads, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop=0., attn_drop=0., drop_path=0., act_layer=leakyrelu, norm_layer=nn.LayerNorm, window_size=16):
        super().__init__()
        self.depth = depth
        models = [Block(
                        dim=dim,
...
class MakeDense(nn.Module):
    def __init__(self, in_channels, growth_rate, num_heads,mlp_ratio=4., qkv_bias=False, qk_scale=None, drop=0., attn_drop=0., drop_path=0., act_layer=leakyrelu, norm_layer=nn.LayerNorm, window_size=16):
...
class Generator_tailadd(nn.Module):
    ...
    def forward(self, x, gsfeatures, gsrgb):
        ...
        # 特征融合
        fu_rgb = tfrgb[0] + gsrgb[0]  # RGB融合
        fu_feature = features[0] + gsfeatures[0]  # 特征融合
